...
bool rv::graph_builder::remove_node(std::uint16_t _id) {
	if (m_nodes.empty() || m_nodes.front().id == _id)
		return false;
	...
	for (graph_builder_node& node : m_nodes) {
		for (graph_builder_pin_links& pin_links : node.links)
			std::erase_if(pin_links.targets, [&](const graph_builder_pin_link_target& _target) {
				return _target.node_id == _id;
			});
	}
...
bool rv::graph_builder::remove_selected_node() {
	const graph_builder_node* node = selected_node();
	if (node == nullptr)
		return false;
