...
ui_render_result ui_render(const std::vector<std::unique_ptr<rv::function_instance>>& _functions, std::size_t _active_function_index, const std::vector<std::unique_ptr<rv::variable>>& _variables, selection_manager& _selection, graph_builder& _graph, bool _running, const type_registry& _type_registry);
